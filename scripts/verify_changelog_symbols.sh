...
while IFS= read -r bullet; do
    ...
    while IFS= read -r token; do
        ...
        # Skip non-identifier tokens (e.g. `=,+,-,@`)
        ...
        if echo "$CLEAN_TOKEN" | grep -qE "^($WHITELIST)$"; then
            echo "     ℹ️  Skipped whitelisted token: '$CLEAN_TOKEN'"
            continue
        fi
        ...
        if [ -n "$PREFIX" ] && [ -d "$PREFIX" ]; then
            if git grep -qw "${CLEAN_TOKEN}" -- "$PREFIX/*.go" 2>/dev/null; then
                FOUND=1
            fi
        fi
        if [ "$FOUND" -eq 0 ]; then
            if git grep -qw "${CLEAN_TOKEN}" -- "*.go" 2>/dev/null; then
                FOUND=1
            fi
        fi
    ...
    done < <(echo "$bullet" | grep -oE '`[^`]+`' | tr -d '`' || true)
    ...
    for token in $CAMEL_TOKENS; do
        ...
        FOUND=0
        if [ -n "$PREFIX" ] && [ -d "$PREFIX" ]; then
            if git grep -qw "${token}" -- "$PREFIX/*.go" 2>/dev/null; then
                FOUND=1
            fi
        fi
        if [ "$FOUND" -eq 0 ]; then
            if git grep -qw "${token}" -- "*.go" 2>/dev/null; then
                FOUND=1
            fi
        fi
